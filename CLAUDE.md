...
| Notificaciones por correo (docs/05) | **Construido** | `core/src/ops/notificaciones.ts` (puerto + plantillas puras, `TipoCorreo` = lista/fallo/revision/**invitacion**; la invitación es el ÚNICO correo cuyo destinatario NO son los admins —va al invitado, que aún no tiene cuenta— y por eso `EventoCorreo.destinatario`; el link va a `/registrarse`, público, no al portafolio que el middleware le negaría) + el notificador real con Resend en `web/lib/automata/wiring.ts` (resuelve los correos de los admins por Clerk), inyectado en los **tres** drainers (cosecha, disparo, ajustes) | `verify:notificaciones` |
...
| 2 | **Selector multi-org** (`orgActual()` toma `orgs[0]` fijo). | Quien pertenezca a dos equipos ve SOLO el primero, sin señal de que hay otro. |
| 3 | **Renombrar la org** autogenerada ("Mi negocio"). | No hay endpoint; el nombre sale en la UI y en los correos. |
| 4 | **Re-verificación de MFA en el front** cuando el `fva` envejece. | Mitigado (el mensaje ya da salida: volver a entrar), no resuelto. Afecta invitar/quitar gente **y ahora también pagar** (`facturacion` pide step-up). |
| 5 | **Formula injection** en salidas xlsx/csv (`=`,`+`,`@`) sin neutralizar. | Exposición baja hoy porque lo que se sirve es JSON; cerrar antes de servir hojas de cálculo. |
| 6 | **`correrRegresion()` devuelve `"indeterminado"`** hasta que exista el runner. | Nada se clasifica como reparación gratis automáticamente. La UI lo dice de frente y el ajuste exige `confirmado:true`. |
