...
| 2 | **Renombrar la org** autogenerada ("Mi negocio"). | No hay endpoint; el nombre sale en la UI y en los correos. |
| 3 | **Re-verificación de MFA en el front** cuando el `fva` envejece. | Mitigado (el mensaje ya da salida: volver a entrar), no resuelto. Afecta invitar/quitar gente **y ahora también pagar** (`facturacion` pide step-up). |
| 4 | **Formula injection** en salidas xlsx/csv (`=`,`+`,`@`) sin neutralizar. | Exposición baja hoy porque lo que se sirve es JSON; cerrar antes de servir hojas de cálculo. |
| 5 | **`correrRegresion()` devuelve `"indeterminado"`** hasta que exista el runner. | Nada se clasifica como reparación gratis automáticamente. La UI lo dice de frente y el ajuste exige `confirmado:true`. |
