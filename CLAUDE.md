...
| 3 | **Formula injection** en salidas xlsx/csv (`=`,`+`,`@`) sin neutralizar. | Exposición baja hoy porque lo que se sirve es JSON; cerrar antes de servir hojas de cálculo. |
| 4 | **`correrRegresion()` devuelve `"indeterminado"`** hasta que exista el runner. | Nada se clasifica como reparación gratis automáticamente. La UI lo dice de frente y el ajuste exige `confirmado:true`. |
